...
public class Ufo : MonoBehaviour
{
    ...
    private void Shoot()
    {
        ...
        if (random <= 0)
        {
            Instantiate(forkBullet, new Vector3(transform.position.x, transform.position.y + 1.5f, transform.position.z), Quaternion.identity);
        }
        if (random >= 1)
        {
            Instantiate(knifeBullet, new Vector3(transform.position.x, transform.position.y + 1.5f, transform.position.z), Quaternion.identity);
        }
    }
    void LateUpdate()
    {
